    ...
    while True:
        try:
            ...
            if frame is not None and gaze:
                ...
                # only pass the region around the gaze point to the object detection model
                crop_size = 300  # smaller value = lower computation, but potentially misses objects
                ...
                # extract the cropped image from the original frame
                ...
                try:
                    ...
                    for result in results:
                        for obj in result.boxes:
                            x1, y1, x2, y2 = map(int, obj.xyxy[0]) # bounding box coordinates
                            # translate bounding box coordinates back to the original frame
                            x1 += crop_x1
                            y1 += crop_y1
                            x2 += crop_x1
                            y2 += crop_y1
                            cls = model.names[int(obj.cls[0])] # get class label

                            # check if the gaze point falls inside the bounding box
